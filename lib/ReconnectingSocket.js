...
const CLOSE_NONCE = 1013;
const CLOSE_UNEXPECTED_RETRY = 1014;
  ...
  this.instance.on('unexpected-response', (request, response) => {
    ...
    response.on('end', () => {
      ...
      if (response.statusCode === 401) {
        ...
        this.logError(`Server responded with [${response.statusCode}] (maintainence / downtime), will retry soon: ${buf}`);
        this.autoReconnectInterval = 5000; // first retry in 5 seconds
        closeConnection(CLOSE_DOWNTIME);
      } else if (response.statusCode === 400) {
        // nonce is not increasing
        this.logError(`Server responded with [${response.statusCode}] (nonce is not increasing), will retry soon: ${buf}`);
        closeConnection(CLOSE_NONCE);
      } else {
        this.logError(`Unexpected response from server [${response.statusCode}], will retry soon: ${buf}`);
        this.autoReconnectInterval = 5000; // first retry in 5 seconds
        closeConnection(CLOSE_UNEXPECTED_RETRY);
      // } else {
